...
// `ORBI_USER_DATA_DIR` só existe para rodar uma cópia de desenvolvimento/teste
// com dados separados, sem tocar nas sessões reais do Orbi instalado.
app.setPath('userData', process.env.ORBI_USER_DATA_DIR || path.join(app.getPath('appData'), USER_DATA_DIR_NAME));
...
app.whenReady().then(() => {
  ...
  registerIpcHandlers({
    ...
    // A largura/posição da barra e o tamanho dos ícones continuam sendo
    // gravados em settings.json pelo ipcRouter; a área da instância que
    // resulta deles é medida pelo renderer e chega por `applyContentBounds`.
    applyContentBounds: (rect) => windowManager?.applyContentBounds(rect),
    windowControls: {
      minimize: () => windowManager?.minimize(),
      toggleMaximize: () => windowManager?.toggleMaximize(),
      close: () => windowManager?.close(),
      isMaximized: () => windowManager?.isMaximized() ?? false,
    },
