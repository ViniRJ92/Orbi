...
const api = {
  ...
  setAccountColor: (id: string, color: string) => ipcRenderer.invoke('mw:set-account-color', { id, color }),
  setGroupColor: (id: string, color: string) => ipcRenderer.invoke('mw:set-group-color', { id, color }),
  ...
  /** Retângulo do elemento reservado para a instância (ver windowManager.applyContentBounds). */
  setContentBounds: (rect: { x: number; y: number; width: number; height: number }) =>
    ipcRenderer.send('mw:set-content-bounds', rect),
  windowMinimize: () => ipcRenderer.invoke('mw:window-minimize'),
  windowToggleMaximize: () => ipcRenderer.invoke('mw:window-toggle-maximize'),
  windowClose: () => ipcRenderer.invoke('mw:window-close'),
  windowIsMaximized: () => ipcRenderer.invoke('mw:window-is-maximized'),
  onWindowMaximizedChanged: (cb: (maximized: boolean) => void) => {
    const listener = (_evt: unknown, maximized: boolean) => cb(maximized);
    ipcRenderer.on('mw:window-maximized-changed', listener);
    return () => ipcRenderer.removeListener('mw:window-maximized-changed', listener);
  },
