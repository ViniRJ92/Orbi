...
export interface IpcRouterDeps {
  ...
  /** Retângulo (em px da janela) medido pelo renderer para a instância visível. */
  applyContentBounds: (rect: { x: number; y: number; width: number; height: number }) => void;
  /** Janela sem moldura: minimizar, maximizar e fechar vêm do cabeçalho do Orbi. */
  windowControls: {
    minimize: () => void;
    toggleMaximize: () => void;
    close: () => void;
...
export function registerIpcHandlers(deps: IpcRouterDeps): void {
  ...
  ipcMain.handle('mw:get-diagnostics', () => {
    ...
    return {
      ...
      processCount,
      // Memória física total do computador — referência para as barras de
      // proporção do diagnóstico (o consumo em si é o `memoryBytes` acima).
  ...
  ipcMain.handle('mw:set-sidebar-width', (_evt, width: number) => settingsStore.setSidebarWidth(width));

  // Fase 21/58: posição da barra de contas (Esquerda, Direita, Topo, Inferior).
  ...
  // Área da instância medida pelo renderer (ver windowManager.applyContentBounds).
  ipcMain.on('mw:set-content-bounds', (_evt, rect: { x: number; y: number; width: number; height: number }) => {
    if (!rect || typeof rect.width !== 'number' || typeof rect.height !== 'number') return;
    deps.applyContentBounds(rect);
  });

  // Controles da janela sem moldura.
  ipcMain.handle('mw:window-minimize', () => {
    deps.windowControls.minimize();
    return true;
  });
  ipcMain.handle('mw:window-toggle-maximize', () => {
    deps.windowControls.toggleMaximize();
    return deps.windowControls.isMaximized();
  });
  ipcMain.handle('mw:window-close', () => {
    deps.windowControls.close();
    return true;
  });
  ipcMain.handle('mw:window-is-maximized', () => deps.windowControls.isMaximized());
