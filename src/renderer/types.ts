...
export interface DiagnosticsInfo {
  ...
  cpuPercent: number;
  processCount: number;
  /** Memória física total do computador (referência das barras do diagnóstico). */
...
// Novo layout: cores das categorias conforme a tela "Novo compromisso" do
// Stitch. Os ids (o que fica gravado em calendar.json) não mudaram.
export const EVENT_CATEGORIES = [
  { id: 'trabalho', label: 'Trabalho', color: '#00F1FD' },
  { id: 'reuniao', label: 'Reunião', color: '#A855F7' },
  { id: 'pessoal', label: 'Pessoal', color: '#45F99C' },
  { id: 'atendimento', label: 'Atendimento', color: '#F59E0B' },
  { id: 'outro', label: 'Outro', color: '#859587' },
] as const;
...
export interface OrbiSwitStackApi {
  ...
  /** Retângulo do elemento que reserva a área da instância (em px da janela). */
